...
import { type IColumn, type IProject, type ITask, initialTasks } from './data'
...
const updateOrder = (tasks: ITask[], project: IProject['id'], status: IColumn['status']) => {
  const [projectTasks, otherTasks] = partition(tasks, (t) => t.project === project && t.status === status)

  const orderedTasks = projectTasks.sort((a, b) => a.order - b.order).map((t, i) => ({ ...t, order: i + 1 }))
...
export const TasksProvider = ({ children }: { children: React.ReactElement | React.ReactElement[] }) => {
  ...
  return <TasksContext.Provider value={{ tasks, addTask, removeTask, updateTask }}>{children}</TasksContext.Provider>
}
